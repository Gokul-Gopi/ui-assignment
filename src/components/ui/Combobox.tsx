...
import { CheckIcon, ChevronDownIcon, DotIcon, XIcon } from "lucide-react";
...
function ComboboxInput({
  ...
  return (
    <InputGroup className={cn("w-auto bg-white!", className)}>
      <ComboboxPrimitive.Input
        render={
          <InputGroupInput
            disabled={disabled}
            className="border-none px-4 focus-visible:ring-0 focus-visible:ring-offset-0"
          />
        }
        {...props}
      />
      <InputGroupAddon align="inline-end">
        {showTrigger && (
          <InputGroupButton
            ...
            className="group-has-data-[slot=combobox-clear]/input-group:hidden hover:bg-white! active:bg-white! data-pressed:bg-white!"
            disabled={disabled}
          >
            <ComboboxTrigger />
          </InputGroupButton>
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        `data-highlighted:bg-accent data-highlighted:text-accent-foreground aria-selected:border-primary! relative flex w-full cursor-default items-center justify-between gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none aria-selected:border data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4`,
        className,
      )}
      {...props}
    >
